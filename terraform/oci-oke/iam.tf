...
resource "oci_identity_dynamic_group" "oke_worker_instances" {
  ...
  name           = "oke-worker-instances"
  description    = "OKE worker nodes"
...
resource "oci_identity_policy" "oke_worker_storage" {
  compartment_id = var.tenancy_ocid
  name           = "oke-worker-storage"
  description    = "Allow OKE workers to manage block volume attachments for node storage"

  statements = [
    "Allow dynamic-group oke-worker-instances to manage instance-family in tenancy",
    "Allow dynamic-group oke-worker-instances to use volume-family in tenancy",
  ]
}

resource "oci_identity_policy" "oke_worker_vault_secrets" {
  compartment_id = var.tenancy_ocid
  name           = "oke-worker-vault-secrets"
  description    = "Allow OKE workers to read OCI Vault secret bundles for External Secrets Operator"

  statements = [
    "Allow dynamic-group oke-worker-instances to read secret-bundles in compartment id ${var.compartment_ocid}",
  ]
}
